...
class SisifoInciOtras extends SisifoIncidencia{
   ...
   */       
	...
	*/           
    function SisifoInciOtras ( $id, $crear = false ) {    	
	...
	$this -> rutatmp	= $this -> sisifoConf -> getRutaTmp(); 
	   ...
	   if ( ! $crear ) {
		...
		$sql = "SELECT * FROM inci_otras WHERE id ='" . $id ."'";
    ...
    */
	...
	$db = $this -> sisifoConf -> getBd();
		...
		$db -> StartTrans();
			$last_id = parent::insertar ($desc_breve, $desc_larga, 7, $cc );
		$db -> CompleteTrans();
		return $last_id;
    }
